...
namespace GoFish
{
    class Player
    {
        ...
        public IEnumerable<Values> PullOutBooks()
        {
            ...
            {
                Values value = (Values)i; int howMany = 0;
                for (int card = 0; card < cards.Count; card++) if (cards.Peek(card).Value == value) howMany++;
                if (howMany == 4)
                {
                    books.Add(value);
        ...
        public void AskForACard(List<Player> players, int myIndex, Deck stock)
        {
            if (stock.Count > 0)
            {
                ...
                Values randomValue = GetRandomValue(); AskForACard(players, myIndex, stock, randomValue);
            }
        }
        ...
        public void AskForACard(List<Player> players, int myIndex, Deck stock, Values value)
        {
            ...
            for (int i = 0; i < players.Count; i++)
            {
                if (i != myIndex)
                {
                    ...
                    Deck CardsGiven = player.DoYouHaveAny(value); totalCardsGiven += CardsGiven.Count; while (CardsGiven.Count > 0) cards.Add(CardsGiven.Deal());
                }
